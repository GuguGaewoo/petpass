...
LIST_FILE = "pet_list.json"
DETAIL_FILE = "pet_detail.json"
PROGRESS_FILE = "pet_progress.json"
TIMEOUT = 15
SLEEP = 0.15
...
def call(base, op, **params):
    ...
    if not SERVICE_KEY:
        raise RuntimeError(
            "인증키를 찾을 수 없습니다.\n"
            "  tour_probe.py 와 같은 폴더에 .env 파일을 만들고 아래 한 줄을 넣으세요:\n"
            '    TOUR_API_KEY=여기에인증키붙여넣기\n'
            "  (.gitignore 에 .env 추가하는 것 잊지 마세요)"
        )
