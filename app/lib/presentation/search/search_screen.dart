  ...
  Widget build(BuildContext context) {
    ...
    final list = s.places.where((p) {
      ...
      if (_query.isNotEmpty &&
          ...
          !p.address.contains(_query)) return false;
      return true;
    }).toList();

    final judged = [
      for (final p in list) (place: p, verdict: s.judge(p)),
    ];
    if (_hidePossibleOnly) {
      judged.removeWhere((e) => e.verdict.level != VerdictLevel.possible);
    }
    judged.sort((a, b) => a.verdict.level.index.compareTo(b.verdict.level.index));
            ...
            child: Column(children: [
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  style: const TextStyle(fontFamilyFallback: T.kr, fontSize: 15),
                  decoration: InputDecoration(
                    hintText: '장소 이름이나 주소로 검색',
                    prefixIcon: const Icon(Icons.search, size: 20, color: T.mute),
                    filled: true,
                    fillColor: T.card,
                    isDense: true,
              ...
              SizedBox(
                ...
                child: ListView(
                  ...
                  children: [
                    _filter('가능한 곳만', _hidePossibleOnly,
                        () => setState(() => _hidePossibleOnly = !_hidePossibleOnly)),
              ...
              Expanded(
                child: judged.isEmpty
                    ? const Center(
                        child: Text('조건에 맞는 곳이 없습니다. 필터를 줄여보세요.',
                            style: TextStyle(
                                fontFamilyFallback: T.kr,
                                fontSize: 14,
                                color: T.inkSoft)))
                            ...
                            onTap: () => Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (_) => PlaceDetailScreen(
                                    state: widget.state, place: e.place),
                              ),
                            ),
                          );
                        },
...
class _PlaceCard extends StatelessWidget {
  const _PlaceCard(
      {required this.place, required this.verdict, required this.onTap});
